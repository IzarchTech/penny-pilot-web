...
function LinkItem({ path, title }: Readonly<{ path: string; title: string }>) {
  const pathname = usePathname();

  return (
    <Link
      href={path}
      className={`p-4 hover:bg-gray-800/25 hover:font-semibold hover:dark:bg-background/55 hover:dark:border-background/65 border-l-8 ${pathname === path ? "border-slate-800/45 dark:border-background/45" : "border-transparent"}`}
    >
      {title}
    </Link>
  );
